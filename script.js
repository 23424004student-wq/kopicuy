...
async function loadMenuFromDB() {
    try {
        ...
        if (result.success) {
            console.log('Menu dari database:', result.data);
            // Render menu ke carousel
            renderMenuCarousel(result.data);
        }
    } catch (error) {
...
async function loadStatistics() {
    try {
        ...
        const result = await response.json();
        if (result.success) {
            updateCounterValues(result.data);
        }
    } catch (error) {
